...
const APP = {
  ...
  recentEventCacheMs: 60 * 1000,
  recentDiscoveryBudgetMs: 12000,
...
function isEventCompletedPast(event) {
  const today = startOfTodayMs();
  const eventState = String(event?.state || '').toLowerCase();
  if (eventState === 'active') return false;
  const endsMs = parseEventDateToMs(event?.ends_on);
  if (endsMs) return endsMs < today;
  const startsMs = parseEventDateToMs(event?.starts_on);
  return !!startsMs && startsMs < today;
}

function recentEventSortValue(candidate) {
  const eventEndMs = parseEventDateToMs(candidate?.event?.ends_on) || 0;
  const lastCompletedMs = candidate?.selection?.lastCompleted?.epochMs || 0;
  const eventId = Number(candidate?.event?.id || 0);
  return [eventEndMs, lastCompletedMs, eventId];
}
...
function renderHeadline(snapshot) {
  ...
  if (snapshot.view === 'recent-event') {
    els.headlineBlock.innerHTML = `<div><div class="headline-main">Most recent competition</div><div class="headline-sub">${escapeHtml(snapshot.eventName || 'Most recent completed event')}</div></div>`;
    return;
  }
  ...
  els.headlineBlock.innerHTML = `<div><div class="headline-main">No current event found</div><div class="headline-sub">Checking supported competitions again later.</div></div>`;
}
...
function updateBadge(view) {
  ...
  } else if (view === 'recent-event') {
    els.liveBadge.classList.add('complete');
    els.liveBadge.textContent = 'Recent';
  } else if (view === 'error') {
...
function computeIdleSnapshot(playerName, diagnostics, delayMs) {
  return {
    ...
    scheduleHint: 'Scanning supported competitions for a current event every 72 hours.',
...
async function discoverMostRecentCompletedEvent(playerName) {
  const cacheKey = normalizeName(playerName);
  const cached = readTimedCache('recent-event', cacheKey, APP.recentEventCacheMs);
  if (cached) return cached;

  const playerNorm = normalizeName(playerName);
  const checked = [];
  const candidates = [];
  const deadlineAt = Date.now() + APP.recentDiscoveryBudgetMs;
  ...
  const listResults = await mapWithConcurrency(listJobs, APP.maxParallelSubdomains, async ({ subdomain, delta }) => {
    ...
    try {
      const payload = await fetchJson(listUrl, { ttlMs: APP.listCacheMs, cacheGroup: 'list' });
      const rankedItems = rankCompetitionItems(payload.items || []).sort((a, b) => {
        const aEnd = parseEventDateToMs(a?.ends_on) || 0;
        const bEnd = parseEventDateToMs(b?.ends_on) || 0;
        return bEnd - aEnd || ((b?.id || 0) - (a?.id || 0));
      });
      return { subdomain, delta, listUrl, items: rankedItems };
    } catch (error) {
      return { subdomain, delta, listUrl, items: [], skipped: true, error: error.message };
    }
  });
  ...
  for (const result of listResults) {
    ...
    await mapWithConcurrency(result.items, APP.maxParallelEventsPerList, async (item) => {
      ...
      try {
        ...
        if (!isEventCompletedPast(event)) return null;

        const match = findMatchingTeam(event, playerNorm);
        if (!match) return null;

        const selection = selectGamesForEvent(event, match.team);
        if (!selection.lastCompleted) return null;
  ...
  candidates.sort((a, b) => {
    const [aEnd, aLast, aId] = recentEventSortValue(a);
    const [bEnd, bLast, bId] = recentEventSortValue(b);
    return bEnd - aEnd
      || bLast - aLast
      || bId - aId
      || (b.match.score - a.match.score);
  ...
  const result = { checked, candidates, timedOut: Date.now() >= deadlineAt };
  writeTimedCache('recent-event', cacheKey, result);
  return result;
}
...
function buildRecentEventSnapshot(playerName, candidate, diagnostics) {
  const snapshot = buildSnapshotFromCandidate(playerName, candidate, diagnostics);
  return {
    ...snapshot,
    view: 'recent-event',
    activeGameId: null,
    nextGameId: null,
    nextCheckAt: Date.now() + APP.idleScanMs,
    nextGameLabel: 'No current event scheduled',
    timelineHint: 'Showing the latest completed game from the most recent competition found.',
    scheduleHint: 'Showing completed games from the most recent competition found.',
    lastUpdatedLabel: formatClock(Date.now())
  };
}
    ...
    if (!discovery.candidates.length) {
      ...
      if (discovery.timedOut) {
        ...
        if (recentDiscovery.candidates.length) {
          ...
          const recentDiagnostics = buildDiagnostics({
            phase: 'recent-match',
            reason,
            playerName: state.playerName,
            checkedCurrent: discovery.checked,
            checkedRecent: recentDiscovery.checked,
            matchedCurler: recentChosen.match.curler.name,
            matchedTeam: recentChosen.match.team.name,
            sourceSubdomain: recentChosen.subdomain,
            matchScore: recentChosen.match.score,
            eventId: recentChosen.event.id,
            eventName: recentChosen.event.name,
            completedGameId: recentChosen.selection.lastCompleted?.gameId || null,
            completedGameAt: recentChosen.selection.lastCompleted?.epochMs || null,
            payloadStructure: {
              stagesCount: (recentChosen.event.stages || []).length,
              drawsCount: (recentChosen.event.draws || []).length,
              gamesCount: recentChosen.selection.diagnostics.totalStageGames,
              drawGameRefs: recentChosen.selection.diagnostics.totalDrawGameRefs
            }
          });

          const recentSnapshot = buildRecentEventSnapshot(state.playerName, recentChosen, recentDiagnostics);
          render(recentSnapshot);
